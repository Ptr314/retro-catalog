...
**Types are never checked at run time** — Node strips them. The code is nonetheless kept clean under `tsc` strict mode; to check, install `typescript` and `@types/node` somewhere *outside* the repo (it must stay free of `node_modules`) and run `tsc` with `noEmit`, `strict`, `module`/`moduleResolution: nodenext`, `allowImportingTsExtensions`, `erasableSyntaxOnly`, pointing `files` at `server.ts`, `tools/user.ts` and `tools/import.ts`. The `return notFound(ctx);` idiom in handlers (returning a void call to exit early) is deliberate and passes `tsc`; JetBrains flags it as "Void function return value is used", which is why that one inspection is switched off in `.idea/inspectionProfiles`.
...
**Emulator handoff.** A slot (`program_emulator_files`) launches in one of two ways, and `launchUrlFor()` picks: an **uploaded file** is a package, so `{siteUrl}/files/{name}` goes URL-encoded into the emulator's `{url}` template; a **typed `file_url`** is already a finished launch page and is redirected to verbatim, template untouched. The file wins when both are set. A row therefore no longer implies a file — one left with neither is deleted (`dropEmptySlot`), because the launch button is drawn from the presence of a row. `GET /run/:slug/:emu` bumps both counters (`programs.runs` and the slot's own) and answers 302 — never 301, which browsers would cache and freeze the counter. The emulator fetches the file itself cross-origin, which is why `/files/` and `/screenshots/` carry CORS headers and `sendFile` implements `Range`.
...
- **Light belongs to screens and to things you can press.** Image housings (`.card-shot`, `.program-shot`, `.family-head-shot`, `.family-row-shot`) are the only rounded elements (`--tube`); they carry a faint glare and vignette on `::after`, and images inside use `image-rendering: pixelated` because 8-bit screenshots must scale as pixels. Amber (`--accent`) is phosphor: headings, data, outlines. Two warm signals sit above it and must not be swapped: gold (`--action`, with `--action-deep` and `--glow-action`) fills the primary key and nothing else, while safety orange (`--hot`) is reserved for live lamps and the "author wanted" mark — the power lamp, the home-page cursor, the badge and its note. If gold shows up on something that cannot be pressed, or orange on something that is not a status, it is wrong.
...
- **Nothing overlays the content.** The housing decorates the *frame*, never the picture inside it: no scanline grid, no tint, no dimming filter on `.shot`. Scanlines were tried and removed — over pixel art they read as moiré rather than as a CRT, worst of all in the small tiles. A screenshot and an empty `.shot-empty` placeholder are both shown plain; what says "screen" is the rounded glass and the bezel.
...
- `programs.promoted` floats a program to the top of the «Сначала новые» order (`SORTS.new` in `db.ts`, also the admin list); other orders ignore it.
...
- Download and run counters skip logged-in administrators (`if (!ctx.user)` in `/dl/` and `/run/`): checking your own upload is not an audience. `ctx.user` is filled from the session cookie on every route, not only the admin ones. To test counting by hand, use a private window or `curl` without the cookie.
